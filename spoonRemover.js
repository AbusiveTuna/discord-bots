...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  if (!isSpoon || user.id === client.user.id || user.bot) return;
  ...
  const nameToCheck = [member?.nickname, user.username].filter(Boolean).join(' ').toLowerCase();

  if (nameToCheck.includes('imago')) {
    try {
